...
first_date <- as.Date(returns[which(!is.na(returns$CABK))[1], "Date"])
cat("The first effective data point will thus be", as.character(first_date))
...
banks_to_exclude <- exclude_high_missing(miss_values, 0.1)
...
vix <- prepare_vix(vix, returns)

# Setting aside Data for Test ---------------------------------------------
split_data <- split_backtest_set(returns, vix, test_days = test_size)

returns <- split_data$returns
returns_backtest <- split_data$returns_backtest
vix <- split_data$vix
save(vix, file = "Saved Results/vix.Rds")
vix_backtest <- split_data$vix_backtest
save(vix_backtest, file = "Saved Results/vix_backtest.Rds")
...
banks_to_plot <- c('BNP', 'DBK', 'HSBC', 'SAN')
vol_cl_data <- volatility_clustering(returns, returns_backtest, price_columns, banks_to_plot)

png(paste0(plot_folder, data_type, "_vol_clustering.png"), width = 1400, height = 800, res = 200, bg = "transparent")
plot(vol_cl_data,
     main = NULL,
...
# Regime Fitting ----------------------------------------------------------
result <- run_vix_regime_detection(vix, seed = 7, returns, data_type)
vix <- result$vix
returns <- result$returns
regimes <- result$regimes
save(regimes, file = "Saved Results/regimes.Rds")

plot_vix_with_regimes(vix, data_type, plot_folder, show_title = F, legend_text_size = 14)

# GARCH -------------------------------------------------------------------
# defining GARCH(1,1) specification

garch_result <- fit_garch_models(returns, price_columns, armaorder = c(1, 0), 
                                 dist_grch = "sstd")
...
plot_bank_qqplot(garch_fits, std_residuals, data_type, plot_folder, "HSBC", qqplot_garch_sstd)
...
length(regimes) == nrow(u)
u_regime1 <- u[regimes == 1, ]
u_regime2 <- u[regimes == 2, ]
...
compare_tree_dependence(rvine_single, cvine_single)
...
df_cvine <- res_ic$df$df_cvine
c(df1 + df2, df_single, df_cvine) %>% round()

ll_single <- res_ic$ll$ll_single
ll_regimes <- numeric(length(ll_single))
ll_regimes[regimes == 1] <- res_ic$ll$ll_r1
ll_regimes[regimes == 2] <- res_ic$ll$ll_r2
...
roll_rvines_backtest <- rolling_rvines(returns = returns_backtest, rolling_window = 250, step_size = 250, 
                                       compute_u = TRUE, armaorder = c(1, 0), price_columns = price_columns, 
                                       dist_grch = "sstd")

plot_roll_rvine(roll_rvines, indices = c(1, 5, 9, 16), 
...
simulated_logret(rolling = F, N = 1000, price_columns = price_columns, rvine = rvine_single, returns = returns, 
                 dist_grch = "sstd", armaorder = c(1, 0))

fut_states <- HMM_rolling(vix = vix, vix_test = vix_backtest, seed = 7, test_window = test_size, rolling_window = 1000)
save(fut_states, file = "Saved Results/Future_regimes.Rds")

vix_backtest_to_plot <- vix_backtest

if (data_type=="Stock"){
  vix_backtest_to_plot$regime = as.factor(fut_states)
  vix_backtest_to_plot$regime_label <- factor(fut_states,
                                              levels = c(1, 2),
  ...
  vix_backtest_to_plot$regime_label <- factor(fut_states,
                                              levels = c(1, 2),
                                              labels = c("Tranquility", "Crisis"))
}

future_vix_plot(vix_backtest_to_plot, plot_folder, data_type, vix_future_states, 
                show_title = FALSE, legend_text_size = 14)
...
# Saving Plots
save_covar_plot(CoVaR_single$Delta_CoVaR, title = NULL,
                paste0(data_type, "_covar_single.png"), plot_folder, 
                show_title = F, legend_text_size = 12)

save_covar_plot(CoVaR_crisis$Delta_CoVaR, title = NULL,
                paste0(data_type, "_covar_crisis.png"), plot_folder, 
                show_title = F, legend_text_size = 12)

save_covar_plot(CoVaR_tranq$Delta_CoVaR, title = NULL, 
                paste0(data_type, "_covar_tranq.png"), plot_folder,  
                show_title = F, legend_text_size = 12)

save_covar_plot(CoVaR_crisis$Delta_CoVaR - CoVaR_tranq$Delta_CoVaR, 
                "ΔCoVaR: Crisis - Tranquil", 
                paste0(data_type, "_covar_deltaindelta.png"), plot_folder, 
                lower_limit = -0.08, deltaindelta = T, 
                show_title = F, legend_text_size = 12)

head(CoVaR_table_summary(CoVaR_single$Delta_CoVaR, Delta_CoVaR_logret_single))

# most systematically impactful
colMeans(CoVaR_single$Delta_CoVaR, na.rm = T) %>% sort()
# most vulnerable 
rowMeans(CoVaR_single$Delta_CoVaR, na.rm = T) %>% sort()

colMeans(CoVaR_crisis$Delta_CoVaR - CoVaR_tranq$Delta_CoVaR, na.rm = T) %>% sort()

# Backtest Results --------------------------------------------------------

### MODIFY FUNCTION CALLS WITH dist_grch, armaorder, special_armaorder, special_banks,
...
roll_risk_regimes <- rolling_risk_regimes(rvine1 = rvine_regime1, rvine2 = rvine_regime2, returns = returns, 
                                          ...
                                          rolling_window = 1000, N = 1000, 
                                          dist_grch = "sstd", armaorder = c(1, 0))

save(roll_risk_regimes, file = "Saved Results/roll_risk_regimes.RData")
...
save(roll_risk, file = "Saved Results/roll_risk.RData")
...
if (test_size == 500) {
  rvine_list <- list(roll_rvines[[16]][[3]], roll_rvines_backtest[[1]][[3]])
  } else if (test_size == 250){
  rvine_list <- list(roll_rvines[[17]][[3]])
  }
...
roll_risk_rollvines <- rolling_risk_rollrvines(
  ...
  test_window = test_size, rolling_window = 1000, N = 1000, dist_grch = "sstd", armaorder = c(1, 0))

save(roll_risk_rollvines, file = "Saved Results/roll_risk_rollvines.RData")

roll_risk_cvine <- rolling_risk(rvine = cvine_single, returns = returns, returns_test = returns_backtest, 
                          price_columns, test_window = test_size, rolling_window = 1000, N = 1000, 
                          dist_grch = "sstd", armaorder = c(1, 0))

save(roll_risk_cvine, file = "Saved Results/roll_risk_cvine.RData")
...
# Key Observations:
# - Copula-based IR strategy (ir_cvine) outperforms in credit and tail-driven episodes.
# - Regime-switching and copula MinVar strategies excel during volatility spikes.
# - The naïve rolling correlation MinVar strategy (minvar_corr) performs well in linear-stress regimes (e.g., bond shock).
# - Copula-based methods show robustness and tail risk awareness across most stress scenarios.
#
# These results support the use of nonlinear dependence modeling
# (via R-vine copulas) for managing portfolio risk under adverse conditions.
